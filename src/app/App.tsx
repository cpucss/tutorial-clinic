...
const TITLES: Record<TabKey, string> = {
  ...
  points: "Guides / Points Guide",
  "points-history": "Personal / Point History",
  ...
  "admin-subjects": "Admin / Subjects Catalog",
...
export default function App() {
  ...
  const [qrModeOpen, setQrModeOpen] = useState(false);
  const [offline, setOffline] = useState(() => !navigator.onLine);

  const routeTab = tabFromPath(location.pathname);
  const fallback: TabKey = currentUser?.role === "admin" ? "admin-dashboard" : "dashboard";
  const tab = routeTab ?? fallback;
  const title = TITLES[tab] ?? "CCS Tutorial Clinic";
  ...
  useEffect(() => {
    ...
    if (location.pathname === "/login" || !routeTab) {
      navigate(TAB_PATHS[fallback], { replace: true });
      return;
    }
    if (currentUser.role !== "admin" && ADMIN_TABS.includes(routeTab)) {
      navigate(TAB_PATHS.dashboard, { replace: true });
    }
    if (
      currentUser.role === "admin" &&
      !ADMIN_TABS.includes(routeTab) &&
      !["profile", "settings", "notifications", "announcements", "help"].includes(routeTab)
    ) {
      navigate(TAB_PATHS["admin-dashboard"], { replace: true });
    }
  }, [currentUser, fallback, location.pathname, navigate, routeTab]);

  useEffect(() => {
    document.title = `${title.split(" / ").at(-1)} - CCS Tutorial Clinic`;
  }, [title]);
  ...
  useEffect(() => {
    const focusDialog = () => {
      ...
      if (!dialog || dialog.dataset.focusReady) return;
    ...
    const handleKey = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape" && !dialog.classList.contains("confirm-dialog")) {
        const close = dialog.querySelector<HTMLButtonElement>('[aria-label*="Close"], header > button');
        if (close) {
  ...
  function handleNavigate(next: TabKey) {
    if (ADMIN_TABS.includes(next) && currentUser?.role !== "admin") {
      notify({
        ...
        description: "Sign in with an administrator account to open this page.",
      });
      return;
    }
  ...
  function handleLogout() {
    ...
    notify({
      tone: "info",
      title: "Signed out",
      description: "Your session ended on this device.",
    });
  }
  ...
  async function share() {
    try {
      ...
      notify({
        ...
        description: "The hash route is ready to share.",
      });
    } catch {
      notify({
        tone: "warning",
        title: "Clipboard unavailable",
        description: "Copy the current address from the browser bar.",
      });
    }
  }
    ...
    <div className="app-shell flex min-h-dvh w-full">
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-white">
        ...
        <TopBar
          ...
          onShare={share}
        />
        <main id="main-content" className="app-content min-h-0 min-w-0 flex-1 overflow-auto bg-[#FAF8F2]">
      ...
      {qrModeOpen && <QrModeSheet onClose={() => setQrModeOpen(false)} onNotify={notify} />}
      {currentUser.role !== "admin" &&
        currentUser.accountSetup.mustChangePassword !== false &&
        !currentUser.accountSetup.completed &&
        !currentUser.accountSetup.skipped && (
          <AccountSetupModal
            onComplete={(deferred) =>
              notify(
                deferred
                  ? {
                      ...
                      description: "You can change your temporary password from Notifications or Settings.",
                    }
                  : {
                      tone: "success",
...
function Page({
  ...
  switch (tab) {
    ...
    case "attendance-history":
      return <AttendanceHistoryPage onNotify={notify} />;
    case "leaderboard":
      return <LeaderboardPage />;
    case "notes":
      return <NotesPage onNotify={notify} />;
    case "profile":
    ...
    case "my-notes":
      return <MyNotesPage onNotify={notify} />;
    case "settings":
      return <SettingsPage onNotify={notify} />;
    case "admin-dashboard":
      return <AdminDashboardPage onNavigate={navigate} />;
    case "admin-attendance":
      return <AdminAttendancePage onNotify={notify} />;
    case "admin-sessions":
      return <AdminSessionsPage onNotify={notify} />;
    case "admin-subjects":
      return <AdminSubjectsPage onNotify={notify} />;
    case "admin-notes":
      return <AdminNotesApprovalPage onNotify={notify} />;
    case "admin-students":
      return <AdminStudentsPage onNotify={notify} />;
  }
}
