...
export function SettingsPage({ onNotify }: { onNotify?: (toast: Omit<ToastMessage, "id">) => void }) {
  ...
  async function save() { setSavingPreferences(true); const result = await updatePreferences(preferences); setSavingPreferences(false); onNotify?.({ tone: result.ok ? "success" : "error", title: result.ok ? "Preferences saved" : "Preferences not saved", description: result.ok ? "Your display and notification preferences are stored with this account." : result.message }); }
  async function reset() { const defaults = { ...defaultPreferences }; setSavingPreferences(true); const result = await updatePreferences(defaults); setSavingPreferences(false); if (result.ok) setPreferences(defaults); onNotify?.({ tone: result.ok ? "info" : "error", title: result.ok ? "Preferences reset" : "Preferences not reset", description: result.ok ? "Default settings were restored." : result.message }); }
  ...
  async function handleSaveSecurity(e: React.FormEvent) {
    ...
    const res = await completeAccountSetup(setupPassword);
    setSavingSecurity(false);
    if (!res.ok) {
      setSetupError(res.message);
      return;
    }
    onNotify?.({ tone: "success", title: "Password updated", description: "Your account security settings are now up to date." });
    setSetupPassword("");
