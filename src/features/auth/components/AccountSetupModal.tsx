    ...
    <div className="setup-overlay" role="presentation">
      <div ref={panelRef} className="setup-dialog" role="dialog" aria-modal="true" aria-labelledby="setup-title" aria-describedby="setup-description">
        ...
        <div>
          ...
          <p id="setup-description">Before continuing, set up your backup email and password. These will help secure and recover your account once the backend is connected.</p>
        </div>
        <InlineNotice tone="warning" title="Saved on this device">Until the school account service is connected, these details stay in this browser and are not server-verified.</InlineNotice>
        ...
        <form onSubmit={submit} className="setup-form" noValidate>
          ...
          <label><span>Backup email address</span><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@example.com" autoComplete="email" /></label>
          ...
          <button type="submit" className="primary-button setup-submit"><ShieldCheck size={15} /> Save and Continue</button>
        </form>
      </div>
    </div>
  );
