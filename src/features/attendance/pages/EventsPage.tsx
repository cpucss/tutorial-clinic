...
export function EventsPage({ onNotify }: { onNotify?: (toast: Omit<ToastMessage, "id">) => void }) {
  ...
  async function schedule(eventId: string) {
    ...
    onNotify?.({ tone: result.ok ? "success" : "error", title: result.ok ? (saved ? "Removed from schedule" : "Added to My Schedule") : "Schedule not updated", description: result.message || (result.ok ? "Your schedule is up to date." : "Please try again.") });
  }
