...
export function NotificationsPage({
  ...
  const mine = useMemo(() => {
    ...
    if (currentUser?.accountSetup.mustChangePassword && currentUser.accountSetup.skipped) {
      items.push({
        ...
        message:
          "Finish securing your account from Settings. This reminder will close automatically after your password is changed.",
        type: "Account",
  ...
  function openRelated(item: DemoNotification) {
    ...
    if (item.relatedTab) onNavigate?.(item.relatedTab as TabKey, item.relatedId);
  }
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:px-10 lg:py-8">
        <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <p className="page-description">
              Session reminders, attendance decisions, note reviews, points, and account updates.
            </p>
          </div>
        ...
        <div className="mt-6 flex flex-col gap-4 rounded-xl bg-white p-4 demo-card sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-[#6F6F6F]">of {mine.length} notifications</span>
          </div>
        ...
        {visible.length ? (
          <ul className="mt-4 grid gap-2">
            {visible.map((item) => (
              <li
                key={item.id}
                className={`notification-row ${item.readAt ? "is-read" : "is-unread"}`}
              >
                <button className="notification-main" onClick={() => openRelated(item)}>
                  ...
                  <span className="min-w-0 flex-1 text-left">
                    <span className="flex flex-wrap items-center gap-2">
                      ...
                      {isPasswordReminder(item) ? (
                        <StatusBadge status="Action required" />
                      ) : (
                        !item.readAt && <StatusBadge status="Unread" />
                      )}
                    </span>
                    <span className="mt-1 block text-sm text-[#6F6F6F]">{item.message}</span>
                    <span className="mt-2 block text-xs text-[#8A8377]">
                ...
                <div className="notification-actions">
                  {isPasswordReminder(item) ? (
                    <button onClick={() => onNavigate?.("settings")}>Change now</button>
                  ) : item.readAt ? (
                    <button onClick={() => markNotification(item.id, false)}>Mark unread</button>
                  ) : (
                    <button onClick={() => markNotification(item.id, true)}>Mark read</button>
                  )}
                </div>
              </li>
            ))}
          ...
          <div className="mt-6">
            <EmptyState
              title={filter === "Unread" ? "No unread notifications" : "No notifications in this view"}
