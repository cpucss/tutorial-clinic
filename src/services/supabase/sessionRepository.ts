...
function mapSubjectRow(row: SubjectRow): Subject {
  return {
    ...
    curriculumVersion: row.curriculum_version || "2024-2025",
...
export async function upsertSubject(subject: Subject): Promise<{ data: Subject | null; error: string | null }> {
  const payload: Database["public"]["Tables"]["subjects"]["Insert"] = {
    ...
    lec_hours: subject.lecHours ?? 0,
    lab_hours: subject.labHours ?? 0,
    credit_units: subject.creditUnits ?? 0,
    prerequisites: subject.prerequisites ?? [],
    is_elective: subject.isElective ?? false,
    specialization: subject.specialization ?? null,
    curriculum_version: subject.curriculumVersion || "2024-2025",
